  ...
  class BaseQuery {
    ...
    linkedWith(relatedElement) {
      ...
      if (relatedElement && relatedElement[MODEL]) {
        this[LINKED_WITH] = relatedElement;
      ...
      throw new Error(`linkedWith method called with a bad parameter (${relatedElement}).`);
    }
    ...
    async find() {
      ...
      await this.runComplexQuery(async () => {
        results = results.concat(await this[CONNECTOR].find(this));

        return true;
      });

      return results.map((rawResult) => this.applySelectBehaviorOnConnectorResult(rawResult));
    }
    ...
    async runComplexQuery(accumulator) {
      const stream = await this[LINKED_WITH].stream();
      ...
      stream.on('readable', streamReadyResolve);

      await waitForStream;

      let instance = stream.read();

      while (instance !== null) {
        this.linkedWith(instance);
        ...
        this[QUERY.FIELDS.QUERY].slice(baseLength);
