...
const { LIMIT, QUERY, QUERY_OR, SELECT, SKIP, SORT, } = require('../../constants').QUERY.FIELDS;
...
const handleOnOperator = (queryClass, onOperator) => {
  if (onOperator) {
    for (const queryOperator of queryClass[QUERY]) {
      onOperator(queryOperator);
    }
  }
};
...
const handleSelect = (queryClass, onSelect) => {
  if (onSelect && queryClass[SELECT] && queryClass[SELECT].fields && queryClass[SELECT].fields.length > 0) {
    queryClass[SELECT].fields.forEach((fieldSelect) => onSelect(fieldSelect));
  }
};
...
const handleSort = (queryClass, onSort) => {
  if (onSort) {
    for (const sortParams of queryClass[SORT]) {
      onSort(sortParams);
    }
  }
};
