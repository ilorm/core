...
const { InvalidValueError, UnsupportedFeatureError, } = require('../errors');
...
const injectIlorm = (ilorm) => {
  ...
  class BaseField {
     ...
     */
    isValid(value) {
      return !(this[IS_REQUIRED] && (value === null || value === undefined));
    }
    ...
    init(instance, field) {
      ...
      if (value !== undefined) {
        if (!this.isValid(value)) {
          throw new InvalidValueError(`Invalid ${value} for field ${this[NAME]}`);
        }
    ...
    reference({ referencedModel, referencedField, }) {
      ...
      const defineReferencePromise = new Promise(async (resolve, reject) => {
      ...
      ilorm.Relation.DEFINED.push(defineReferencePromise);

      defineReferencePromise.catch((err) => ilorm.events.emit('error', err));
     ...
     */
    getQueryOperations({ query, name, ancestorFields = null, }) {
      const resultQueryOperations = {
        [OPERATIONS.SET]: (params) => {
          if (!this.isValid(params)) {
            throw new InvalidValueError(`${params} is not a valid value for field ${name}`);
          }

          query[FIELDS.UPDATE].push({
            field: this,
            operator: OPERATIONS.SET,
            value: this.castValue(params),
            ancestorFields,
          });
        ...
        [OPERATIONS.IS]: declareOperation({
          query,
          operation: OPERATIONS.IS,
          field: this,
          ancestorFields,
        }),
        [OPERATIONS.IS_NOT]: declareOperation({
          query,
          operation: OPERATIONS.IS_NOT,
          field: this,
          ancestorFields,
        }),
        [OPERATIONS.IS_IN]: declareOperation({
          query,
          operation: OPERATIONS.IS_IN,
          field: this,
          ancestorFields,
          castValue: (params) => params.map(this.castValue),
        }),
        [OPERATIONS.IS_NOT_IN]: declareOperation({
          query,
    ...
    static getFieldDefinition() {
      throw new UnsupportedFeatureError('Schema field need to overload BaseSchema.getFieldDefinition static method');
    }
  }
