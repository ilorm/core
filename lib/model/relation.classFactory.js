...
const { IlormError, QueryError, } = require('../errors');
...
const injectIlorm = (ilorm) => {
  ...
  class Relation {
    ...
    /**
     * Wait for all relation defined in schema to be initialized
     * @returns {Promise<void>} Resolve when init finished
     */
    static waitForRelationToInit() {
      return Promise.all(RELATION_DEFINED);
    }
     ...
     * @param {Model} modelLinked The model linked with the query
     * @returns {void} Return nothing
     */
    applyToQuery({ query, modelLinked, }) {
      ...
      if (model === this.modelB) {
        ...
        throw new QueryError(`Try applying query (${model.getName()}) not bound to the relation` +
        `(${this.modelA.getName()}:${this.modelB.getName}`);
      }

      this.references.forEach((reference) => {
        query[reference[leftOperator]].is(modelLinked[reference[1 - leftOperator]]);
      });
    }
    ...
    static getRelation({ modelSource, modelReferenced, }) {
      ...
      if (!references.get(modelSource).has(modelReferenced)) {
        throw new IlormError(`${modelSource.getName()} does not reference ${modelReferenced.getName()}`);
      }
  ...
  Relation.DEFINED = RELATION_DEFINED;
